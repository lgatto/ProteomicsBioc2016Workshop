...
solution for MS2 data, but does not scale to MS1 data, expecially when
...
1. The upper panel represents the chomatogram of the `r fileName(ms)`
   raw data file, produced with `chomatogram`.
...
(defined by the `iTRAQ4` parameter; other isbaric tags are available).
...
discovery rate theshold can them be converted to an `MSnSet`, where
...
* While tailor for spatial proteomics data, `r Biocpkg("pRoloc")`'s
  machline learning methods are useful in a wide range of cases and
...
* Identification tranfer between acquisitions (label-free): if a
  feautre was not acquired in MS2 in one replicte, it is possible to
  find the ion in MS space based on the M/Z and retention time
  coordinates of the same ion in a replicate where it was
  identified. (An example of this is implemented in the 
  `r Biocpkg("synapter")` package).
...
* It is of course possible to impute missing values (`?impute`). This
  is however not a straighforward thing, as is likely to dramatically
  fail when a high proportion of data is missing (10s of %). But also,
  ...
  * Biologically relevant missing values, resulting from the *absence*
    ...
    distributed in the data and are defined as **missing not at
    random** (MNAR).
...
![RSR KNN and MinDet imputation](./Figures/imp-sim.png)
  ...
  **left-censor** (minimum value (**right**), but not zero, ...)
  method.
...
agains the UniProt webservices.
...
- Processing and analysis or isobaric tagging mass spectrometry with
  `r Biocpkg("isboar")` and `r Biocpkg("MSnbase")`.
...
- `r CRANpkg("protViz") `: Visualizing and Analyzing Mass Spectrometry
...
- Analysis of label-free data from a Synapt G2 (including ion
  mobility) with `r Biocpkg("synatpter")`.
